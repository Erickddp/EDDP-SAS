...
export function buildSystemPrompt(config: PromptConfig): string {
    const { mode, topic, legalContext, queryAnalysis } = config;
    ...
    const modeLabel = mode === "casual" ? "Tú (Amigable)" : "Usted (Profesional)";
    ...
    let rawSchema = RESPONSE_SCHEMAS[schemaMap[effectiveDetail]];
...
1. SELECCIONA UNA BASE PRIMARIA: Identifica el artículo que constituye la fuente directa de la obligación o sanción.
2. BASES DE APOYO: Usa artículos secundarios solo para definiciones, procedimientos o correlaciones necesarias.
...
${depthRule.maxSentences ? `- Resumen: MÁXIMO ${depthRule.maxSentences} oraciones.\n` : ""}
...
2. "primaryBasis": Debe ser el artículo más autoritativo. Explica en "whySelected" por qué es la base principal.
3. "supportingBasis": Asigna roles claros ("definition" | "procedure" | "sanction" | "correlation" | "exception").
4. CITA PRECISA: Cada mención legal debe usar el [ID: ...] correspondiente.
5. NO INVENTES: Si los fragmentos no contienen la respuesta, indícalo claramente en el summary.
`;
